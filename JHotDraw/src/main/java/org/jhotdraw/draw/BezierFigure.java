...
/**
 * A BezierFigure can be used to draw arbitrary shapes using a
 * <code>BezierPath</code>. It can be used to draw an open path or a closed
 * shape.
 * <p>
 * A BezierFigure can have straight path segments and curved segments. A
 * straight path segment can be added by clicking on the drawing area. Curved
 * segments can be added by dragging the mouse pointer over the drawing area.
 * <p>
 * To creation of the BezierFigure can be finished by adding a segment which
 * closes the path, or by double clicking on the drawing area, or by selecting a
 * different tool in the DrawingEditor.
 *
 *
 ...
 * <br>2.2 2007-04-14 Added BezierContourHandle. We fill now open paths as well.
...
public class BezierFigure extends AbstractAttributedFigure {
    ...
    /**
     * The cappedPath BezierPath is derived from variable path. We cache it to
     * increase the drawing speed of the figure.
     */
    private transient BezierPath cappedPath;

    /**
     * Creates an empty <code>BezierFigure</code>, for example without any
     * <code>BezierPath.Node</code>s. The BezierFigure will not draw anything,
     * if at least two nodes are added to it. The <code>BezierPath</code>
     * created by this constructor is not closed.
     */
    public BezierFigure() {
        this(false);
    }
     ...
     * <code>BezierPath.Node</code>s. The BezierFigure will not draw anything,
     * unless at least two nodes are added to it.
     *
     * @param isClosed Specifies whether the <code>BezierPath</code> shall be
     * closed.
     */
     ...
     * ChopDiamondConnector is returned.
     *
    ...
    /**
     * Checks if this figure can be connected. By default filled BezierFigures
     * can be connected.
     */
    @Override
     ...
     * of the BezierFigure. If the BezierFigure has not at least two nodes,
     * nodes are added to the figure until the BezierFigure has at least two
     * nodes.
     */
    @Override
    ...
    /**
     * Returns a path which is cappedPath at the ends, to prevent it from
     * drawing under the end caps.
     */
    ...
    /**
     * Convenience method for getting the point coordinate of the first control
     * point of the specified node.
     */
     ...
     * If the BezierFigure has not at least two nodes, nodes are added to the
     * figure until the BezierFigure has at least two nodes.
     */
    ...
    /**
     * Convenience method for setting the point coordinate of the end point. If
     * the BezierFigure has not at least two nodes, nodes are added to the
     * figure until the BezierFigure has at least two nodes.
     */
    ...
    /**
     * Finds a control point index. Returns -1 if no control point could be
     * found. FIXME - Move this to BezierPath
     */
     ...
     * @param tolerance a tolerance, tolerance should take into account the line
     * width, plus 2 divided by the zoom factor.
